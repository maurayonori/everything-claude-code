...
전체 변경 내역은 [Releases](https://github.com/maurayonori/everything-claude-code/releases)에서 확인하세요.
...
> **참고:** Claude Code 플러그인 시스템은 `rules`를 플러그인으로 배포하지 않습니다. 룰은 수동으로 설치합니다.
>
> **`cp rules/common/* ~/.claude/rules/`처럼 한 폴더로 펼치지 마세요.** 언어별 디렉터리에 같은 이름의 `.md`가 있어 `/*` 복사 시 **덮어쓰기**되고 Markdown의 `../common/` 링크도 깨집니다. `~/.claude/rules/common`, `~/.claude/rules/typescript`처럼 **하위 디렉터리를 유지**하세요.
>
> ```bash
> git clone https://github.com/maurayonori/everything-claude-code.git
> cd everything-claude-code
> npm install   # 또는 pnpm | yarn | bun
>
> # 옵션 A(권장): 사용자 전역 — 스크립트로 common + 나열한 언어
> ./install.sh typescript python golang
>
> # 옵션 A(수동): 디렉터리 단위 복사(소스에 `/*` 없음)
> # mkdir -p ~/.claude/rules
> # cp -R rules/common ~/.claude/rules/common
> # cp -R rules/typescript ~/.claude/rules/typescript
>
> # 옵션 B: 프로젝트(동일한 하위 디렉터리 구조)
> # mkdir -p .claude/rules
> # cp -R rules/common .claude/rules/common
> ```
>
> 자세한 내용: [rules/README.md](../../rules/README.md).
...
# 룰만 (펼치지 말고 하위 디렉터리 유지)
cd everything-claude-code && npm install && ./install.sh typescript && cd ..
# 또는: cp -R everything-claude-code/rules/common ~/.claude/rules/common
```
