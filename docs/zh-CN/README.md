...
* **修复了直觉导入内容丢失问题** — `parse_instinct_file()` 在 `/instinct-import` 期间会静默丢弃 frontmatter 之后的所有内容（Action, Evidence, Examples 部分）。已由社区贡献者 @ericcai0814 修复 ([#148](https://github.com/maurayonori/everything-claude-code/issues/148), [#161](https://github.com/maurayonori/everything-claude-code/pull/161))
...
完整的更新日志请参见 [Releases](https://github.com/maurayonori/everything-claude-code/releases)。
...
**历史背景：** 这已导致此仓库中多次修复/还原循环（[#29](https://github.com/maurayonori/everything-claude-code/issues/29), [#52](https://github.com/maurayonori/everything-claude-code/issues/52), [#103](https://github.com/maurayonori/everything-claude-code/issues/103)）。Claude Code 版本之间的行为发生了变化，导致了混淆。我们现在有一个回归测试来防止这种情况再次发生。
...
>
> **不要使用 `cp rules/common/* ~/.claude/rules/` 将规则「拍平」到一层目录。** 各语言包中有同名文件（如 `testing.md`），用 `/*` 复制到同一目录会**互相覆盖**，并破坏这些 Markdown 里对 `../common/` 的相对链接。请保持**子目录结构**：`~/.claude/rules/common/`、`~/.claude/rules/typescript/` 等（多种语言可以并存）。
>
> ```bash
> git clone https://github.com/maurayonori/everything-claude-code.git
> cd everything-claude-code
...
>
> # 选项 A（推荐）：用户级 — 脚本安装 common + 所列语言
> ./install.sh typescript python golang php   # 按实际使用的语言删减
>
> # 选项 A（手动）：复制整个目录（源路径不要用 `/*` 拍平）
...
> ```
>
> 完整说明见 [rules/README.md](../../rules/README.md)。
...
# Rules：不要拍平目录 — 推荐 install.sh（见 rules/README.md）
...
**钩子：** 若以**插件**方式安装 ECC，`hooks/hooks.json` 会由插件**自动加载**，**不要**再把相同钩子写入 `~/.claude/settings.json`（会触发重复检测）。**仅手动安装、不用插件时**，才把 `hooks/hooks.json` 顶层的 **`"hooks": { ... }`** 合并进 `settings.json`（勿整文件覆盖）。命令里常用 **`${CLAUDE_PLUGIN_ROOT}`**，手动用时请设为该仓库的绝对路径或改写为真实路径。
...
这是最常见的问题。**不要在 `.claude-plugin/plugin.json` 中添加 `"hooks"` 字段。** Claude Code v2.1+ 会自动从已安装的插件加载 `hooks/hooks.json`。显式声明它会导致重复检测错误。参见 [#29](https://github.com/maurayonori/everything-claude-code/issues/29), [#52](https://github.com/maurayonori/everything-claude-code/issues/52), [#103](https://github.com/maurayonori/everything-claude-code/issues/103)。
...
# Just rules（勿拍平 — 保持子目录）
cd everything-claude-code && npm install && ./install.sh typescript && cd ..
# 或: cp -R everything-claude-code/rules/common ~/.claude/rules/common
```
...
* **Codex**: 对 macOS 应用和 CLI 的一流支持，带有适配器漂移防护和 SessionStart 回退。参见 PR [#257](https://github.com/maurayonori/everything-claude-code/pull/257)。
